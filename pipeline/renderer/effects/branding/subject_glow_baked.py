"""Baked subject glow effect - renders glow directly into frames for content-anchoring.

This version bakes the glow into the clip's frames using transform(), ensuring the glow
moves with the content during subsequent pan/zoom operations (content-anchored behavior).

Use this for content-anchored workflows. For frame-anchored workflows, use subject_glow.py.
"""
...
def apply_subject_glow_baked(
    ...
    """Apply multi-stage animated glow baked into frames (content-anchored).

    Renders glow directly into clip frames using transform(), ensuring the glow
    moves with content during subsequent pan/zoom/crop operations.
    ...
    :return: Video clip with glow baked into frames.
    """
    w, h = clip.size
    ...
    if bbox is None and mask is None:
        logger.warning("[subject_glow_baked] No bbox or mask provided, using centered default")
    ...
    def generate_glow_overlay(t: float) -> Image.Image:
        """Generate glow overlay for given time."""
    ...
    def transform_frame(get_frame, t):
        """Bake glow directly into frame."""
        # Get base frame
        base_frame = get_frame(t)
        base_img = Image.fromarray(base_frame)

        # Generate glow overlay
        glow_overlay = generate_glow_overlay(t)

        # Composite glow onto base
        base_rgba = base_img.convert("RGBA")
        result = Image.alpha_composite(base_rgba, glow_overlay)

        # Convert back to RGB
        return np.array(result.convert("RGB"))

    logger.debug(f"[subject_glow_baked] Applying baked glow (content-anchored)")
