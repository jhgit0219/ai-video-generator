            ...
            if callable(detect_anchor):
                try:
                    ...
                    if detected_anchor:
                        ...
                        logger.debug(f"[effects] zoom_on_subject anchor at ({cx:.1f}, {cy:.1f})")
                    else:
                        logger.debug(f"[effects] zoom_on_subject YOLO anchor detection returned None - skipping effect")
                        return clip
                except Exception as e:
                    logger.debug(f"[effects] zoom_on_subject anchor detection failed: {e}")
                    return clip
            if cx is None or cy is None:
                logger.debug(f"[effects] zoom_on_subject no anchor detected - skipping effect")
                return clip
        except Exception as e:
            logger.debug(f"[effects] zoom_on_subject mask center failed: {e}, using bbox center")
            cx = x + bw / 2
            cy = y + bh / 2
